...
function exportToExcel() {
    ...
    // Iterate through selected or all rows
    ...
    // Determine export message based on selection
    const exportMessage = selectedRows.length > 0 
        ? `Exporting ${selectedRows.length} selected item(s)` 
        : 'Exporting all items';
    
    // Confirm export
    const confirmExport = confirm(`${exportMessage}. Continue?`);
...
function setupEditModalEventListeners() {
    ...
    function updateTableRow(row, component, partsName, partsNumber, quantity, itemPrice, date, imageData, rack = 'N/A', tax = '0.00', totalAmount = '0.00', pic = 'N/A', po = 'N/A', ctpl = 'N/A') {
        ...
        // Update data attributes with explicit string values
        row.dataset.rack = rack || 'N/A';
        row.dataset.tax = formatPeso(tax);
        row.dataset.totalAmount = formatPeso(totalAmount);
        row.dataset.pic = pic || 'N/A';
        row.dataset.po = po;  // Explicitly set PO
        row.dataset.ctpl = ctpl;  // Explicitly set CTP
